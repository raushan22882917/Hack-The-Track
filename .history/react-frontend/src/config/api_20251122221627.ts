...
const getWebSocketUrl = (path: string): string => {
  ...
  if (import.meta.env.DEV) {
    ...
    // Use ws: protocol for relative URLs (browser will use current host)
    return `ws://${window.location.host}${normalizedPath}`;
  }
  
  // In production, use absolute URL from BASE_URL
